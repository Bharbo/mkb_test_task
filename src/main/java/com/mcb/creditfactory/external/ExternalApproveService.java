...
public class ExternalApproveService {
    ...
    public int approve(CollateralObject object) {
        ...
        switch (object.getType()) {
            case CAR: code = approveCar(object);
            break;
            case AIRPLANE: code = approvePlane(object);
            break;
            default: code = -100;
        }
        return code;
    ...
    private int approveCar(CollateralObject object) {
        ...
        if (price != null) {
            if (price.getDate().isBefore(MIN_ASSESS_DATE)) {
                return -11;
            }

            if (price.getPrice().compareTo(MIN_CAR_VALUE) < 0) {
                return -12;
            }
        }
    ...
    private int approvePlane(CollateralObject object) {
        ...
        if (price != null) {
            ...
            if (price.getPrice().compareTo(MIN_PLANE_VALUE) < 0) {
                return -22;
            }
        }
        return 0;
